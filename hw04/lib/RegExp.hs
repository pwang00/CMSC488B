...
testName :: Test
testName = "name" ~:
  TestList[ accept name "Leonidas"        ~? "a good name"
          , accept name "L"               ~? "short is ok"
          , not (accept name "leonidas")  ~? "must be capitalized"
          , not (accept name "Leonid4s")  ~? "no extra symbols"
          , not (accept name "Leo Nidas") ~? "not even spaces"
          ]
...
bestCourse :: RegExp
bestCourse = string "cmsc488b" `Alt` string "cmsc631"
...
parts :: [a] -> [[[a]]]
parts [] = [[]]
parts (x:xs) = [[x] : y | y <- parts xs] ++ [(x:z):zs | y@(z:zs) <- parts xs]
...
prop_split :: [Int] -> Bool
prop_split str = all (\(a, b) -> a ++ b == str) $ split str
